...
public:
    ...
    bool WriteZEZYSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZEZYSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZEZYSeed_deprecated(uint256& seed);
    bool EraseZEZYSeed();
    bool EraseZEZYSeed_deprecated();

    bool WriteZEZYCount(const uint32_t& nCount);
    bool ReadZEZYCount(uint32_t& nCount);
