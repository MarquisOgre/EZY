...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EZY);
    ...
    if (!settings.contains("nAnonymizeEazyAmount"))
        settings.setValue("nAnonymizeEazyAmount", 1000);

    nAnonymizeEazyAmount = settings.value("nAnonymizeEazyAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEazyAmount"))
        SoftSetArg("-anonymizeeazyamount", settings.value("nAnonymizeEazyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eazy.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEazyAmount:
            return QVariant(nAnonymizeEazyAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEazyAmount:
            nAnonymizeEazyAmount = value.toInt();
            settings.setValue("nAnonymizeEazyAmount", nAnonymizeEazyAmount);
            emit anonymizeEazyAmountChanged(nAnonymizeEazyAmount);
            break;
        case CoinControlFeatures:
