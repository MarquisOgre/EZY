...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::EZY)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sEZYPercentage, QString& szEZYPercentage)
{
    int nPrecision = 2;
    ...
    szEZYPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sEZYPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelEZYPercent->setText(sPercentage);
    ui->labelzEZYPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zEZY.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in eazy.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in eazy.conf");
    }
    ...
    bool showEZYAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyEZYAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showEZYPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyEZYPending = watchUnconfBalance != 0;
    bool showEZYLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyEZYLocked = nWatchOnlyLockedBalance != 0;
    ...
    bool showzEZYAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzEZYUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzEZYImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzEZYAvailable);
    ui->labelzBalanceMatureText->setVisible(showzEZYAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzEZYUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzEZYUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzEZYImmature);
    ui->labelzBalanceImmatureText->setVisible(showzEZYImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelEZYPercent->setVisible(showPercentages);
    ui->labelzEZYPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("EZY")
    updateDisplayUnit();
}
