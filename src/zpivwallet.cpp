...
CzEZYWallet::CzEZYWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEZYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEZYSeed_deprecated()) {
                    LogPrintf("%s: Updated zEZY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzEZYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEZYCount(nCountLastUsed);
    else if (!walletdb.ReadZEZYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEZYWallet::Lock()
{
    seedMaster = 0;
}

void CzEZYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
...
void CzEZYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEZY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEZYWallet::LoadMintPoolFromDB()
{
...
void CzEZYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEZYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEZYWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzEZYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEZY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzEZYWallet::SeedToZEZY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEZYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEZYWallet::GenerateDeterministicZEZY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEZYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEZYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
