...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 EZY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zEZY send
enum ZerocoinSpendStatus {
    ZEZY_SPEND_OKAY = 0,                            // No error
    ZEZY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEZY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEZY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEZY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEZY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEZY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEZY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEZY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEZY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEZY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEZY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEZY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEZY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEZY_SPENT_USED_ZEZY = 14,                      // Coin has already been spend
    ZEZY_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEZY_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZEZYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzEZYWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzEZYTracker> zpivTracker;
    ...
    void setZWallet(CzEZYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzEZYTracker>(new CzEZYTracker(strWalletFile));
    }

    CzEZYWallet* getZWallet() { return zwalletMain; }
    ...
    /** zEZY reset */
    boost::signals2::signal<void()> NotifyzEZYReset;
